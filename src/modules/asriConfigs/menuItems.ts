...
export function createMobileConfigButton() {
  if (!environment.isMobile) return;
  if (document.getElementById("asri-config-mobile")) return;
  ...
  configBtn.addEventListener("click", (e) => {
    ...
    menu.id = "asri-config-mobile-menu";
    ...
    const asriConfigMenuHTML = `
        ...
        <button class="b3-menu__item asri-config" data-type="nobg" id="asriChroma">
            ...
            <div aria-label="${
              i18n["asriChroma"] + (asriConfigs[curMode].chroma ?? "1")
            }" class="b3-tooltips b3-tooltips__n">
                <input style="box-sizing: border-box" type="range" id="asriChromaSlider" class="b3-slider fn__block" min="0" max="5" step="0.1" value="1">
            </div>
        </button>
        <button class="b3-menu__separator asri-config fn__none" style="margin: 5px 12px;"></button>
        <button class="b3-menu__item asri-config fn__none" id="topbarFusionPlus"></button>
    `;

    menu.innerHTML = `<div class="b3-menu__items">${asriConfigMenuHTML}</div>`;
    ...
    setPickColorBtn(document.getElementById("pickColor"));
    setAsriChromaSlider(document.getElementById("asriChromaSlider") as HTMLInputElement | null);
    setColorPicker(document.getElementById("asriColorPicker") as HTMLInputElement | null);

    initAsriConfigMenuItemClick();

    const closeMenuHandler = (event: MouseEvent) => {
      if (!menu.contains(event.target as Node) && event.target !== configBtn) {
        menu.remove();
